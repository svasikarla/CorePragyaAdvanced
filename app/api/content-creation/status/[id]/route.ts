...
import { contentSseEmitter } from "@/lib/content-creation/store/sse-emitter";
...
export async function GET(
  ...
  let job;
  try {
    job = await contentJobStore.get(id);
  } catch (err) {
  ...
  if (!job) {
    return new Response(
      JSON.stringify({ error: "Job not found. Run the content_creation_jobs migration." }),
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      const unsubscribe = contentSseEmitter.subscribe(id, (event, data) => {
      ...
      const pollTimer = setInterval(async () => {
        const current = await contentJobStore.get(id);
        if (!current) return;
